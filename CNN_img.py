...
def MV_synthesis(tensor, num_filters):
  ...
  with tf.compat.v1.variable_scope("MV_synthesis"):
    with tf.compat.v1.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
...
def Res_analysis(tensor, num_filters, M, reuse=False):
  ...
  with tf.compat.v1.variable_scope("analysis", reuse=reuse):
    with tf.compat.v1.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
...
def Res_synthesis(tensor, num_filters, reuse=False):
  ...
  with tf.compat.v1.variable_scope("synthesis", reuse=reuse):
    with tf.compat.v1.variable_scope("layer_0"):
      layer = tfc.SignalConv2D(
